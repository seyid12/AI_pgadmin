...
class SqlEditorModule(PgAdminModule):
    ...
    def get_exposed_url_endpoints(self):
        ...
        """
        return [
            'sqleditor.generate_sql',   # <--- YENİ EKLENEN SATIR (En başa ekledim)
...
##########################################################################
# PGADMIN AI ENGINE - MULTI-MODE (Üret, Düzelt, Hızlandır, Yorumla)
##########################################################################
##########################################################################
# PGADMIN AI ENGINE - DATA ANALYST EDITION (Veri Okuma Özellikli)
##########################################################################
##########################################################################
# PGADMIN AI ENGINE - FIXED & READY (Veri Okuma Modu)
##########################################################################
##########################################################################
# PGADMIN AI ENGINE - FINAL RESTORED (init Fonksiyonlu)
##########################################################################
# ================================================================
# PGADMIN AI EXTENSION - YAMA (Veri Okuma ve Analiz)
# ================================================================

# ==============================================================================
#  PGADMIN AI & GRAFIK MOTORU (EKLENTI)
#  Bu kismi dosyanin EN ALTINA yapistirin. Yukaridaki orijinal kodlara dokunmayin.
# ==============================================================================

# ==============================================================================
#  AI & GRAFIK MOTORU (EKLENTI)
#  Bu kodlari dosyanin EN ALTINA yapistirin.
# ==============================================================================
...
# ==========================================================
# AI & GRAFİK MOTORU (EKLENTİ - TEMİZ VERSİYON)
# ==========================================================

# --- SADECE EKSİK OLANLARI EKLE (DİĞERLERİ YUKARIDA VAR) ---
import io
import base64
import os
import re
import requests  # <-- EKSİKTİ, EKLENDİ

# --- 1. YARDIMCI FONKSİYONLAR ---

def get_db_connection_ai(trans_id):
    """Veritabanı bağlantısını trans_id üzerinden yakalar."""
    try:
        from pgadmin.tools.sqleditor.utils.query_tool_connection_check import query_tool_connection_check
        res = query_tool_connection_check(trans_id)
...
def get_data_as_df_ai(trans_id, sql_query):
    """SQL sorgusunu çalıştırıp Pandas DataFrame döner."""
    try:
        conn = get_db_connection_ai(trans_id)
        if conn:
            status, result = conn.execute_dict(sql_query)
...
def ask_ai_service(sys_prompt, user_prompt, provider, api_key):
    """AI Servisine (Ollama/Gemini) bağlanır."""
    instruction = "SADECE saf kod döndür. Açıklama yapma. 'İşte sorgun' deme."
    full = f"{sys_prompt}\n{instruction}\n\nUSER REQUEST: {user_prompt}"
    
    try:
        if provider == 'ollama':
            # Docker içinden Windows Ollama'ya erişim adresi
            ollama_url = "http://host.docker.internal:11434/api/generate"
            r = requests.post(
                ollama_url, 
                json={"model": "qwen2.5-coder", "prompt": full, "stream": False}, 
                timeout=120
            )
            ...
            return genai.GenerativeModel('gemini-2.5-flash').generate_content(full).text
    except Exception as e:
        return f"AI Hatası: {str(e)}"

def extract_pure_sql(text):
    """Metin içindeki gereksiz tüm açıklamaları siler, sadece SQL'i bırakır."""
    text = re.sub(r'```(?:sql|python|)?|```', '', text).strip()
    sql_match = re.search(r'(SELECT|INSERT|UPDATE|DELETE|WITH|CREATE|ALTER|DROP).*', text, re.IGNORECASE | re.DOTALL)
    if sql_match:
...
def generate_chart_logic(df, user_prompt, provider, api_key):
    """Grafik çizim mantığı: Hatalar giderildi, boyut büyütüldü ve etiketler düzenlendi."""
    try:
        import matplotlib
        matplotlib.use('Agg')
        ...
        # 1. VERİ TİPİ DÜZELTME
        # Sayı olabilecek sütunları otomatik çevir (Grafik çizimi için şart)
        df = df.apply(pd.to_numeric, errors='ignore')

        # 2. GRAFİK AYARLARI
        plt.close('all')
        plt.figure(figsize=(14, 8)) # Grafiği büyüttük
        sns.set_theme(style="whitegrid")

        cols = list(df.columns)
        sample = df.head(3).to_string()

        # 3. AI TALİMATI (Yazıların sığması için ekleme yapıldı)
        sys_msg = (
            f"Sütunlar: {cols}\n"
            f"İstek: {user_prompt}\n"
            "SADECE Seaborn kodu yaz. 'df' değişkenini kullan. "
            "ÖNEMLİ: Eğer x eksenindeki yazılar (labels) uzunsa, sığması için "
            "'plt.xticks(rotation=45, ha=\"right\")' komutunu ekle. "
            "Grafiğe başlık ve eksen isimleri ekle."
        )

        raw_resp = ask_ai_service(sys_msg, "Grafik kodu yaz.", provider, api_key)

        # 4. KOD TEMİZLEME
        code_match = re.search(r'```python(.*?)```', raw_resp, re.DOTALL)
        code = code_match.group(1).strip() if code_match else raw_resp
        code = re.sub(r'```(?:python|)?|```', '', code).strip()
        code = "\n".join([
            line for line in code.split("\n") 
            if not line.strip().startswith(("Certainly", "Here", "I have", "This", "Sure", "Okay"))
        ])

        # 5. GÜVENLİ ÇALIŞTIRMA (Builtins hatası için düzeltme)
        exec_scope = {
            'df': df, 'sns': sns, 'plt': plt, 'pd': pd,
            '__builtins__': globals()['__builtins__'] 
        }
        exec(code, exec_scope)

        # Yazıların resim dışına taşmasını engeller
        plt.tight_layout()

        # 6. DOSYA KAYDETME
        static_dir = os.path.join(os.getcwd(), 'pgadmin', 'tools', 'sqleditor', 'static')
        if not os.path.exists(static_dir): 
            os.makedirs(static_dir, exist_ok=True)

        img_path = os.path.join(static_dir, 'ai_chart.png')
        # dpi=100 ile netlik artırıldı
        plt.savefig(img_path, format='png', bbox_inches='tight', dpi=100)
        plt.close('all')
        ...
        html = (
            f"<html><body style='text-align:center; font-family:sans-serif; background:#f0f2f5; padding:20px;'>"
            f"<div style='background:white; padding:20px; border-radius:15px; display:inline-block; box-shadow:0 10px 25px rgba(0,0,0,0.1);'>"
            f"<h2 style='color:#2c3e50;'>{user_prompt}</h2>"
            f"<img src='data:image/png;base64,{plot_url}' style='border-radius:8px; max-width:100%;'/>"
            f"</div></body></html>"
        )

        html_path = os.path.join(static_dir, 'ai_chart.html')
        with open(html_path, "w", encoding="utf-8") as f:
            f.write(html)

        return f"-- ✅ Grafik Hazır (Büyük Boy): http://localhost:8080/sqleditor/static/ai_chart.html"

    except Exception as e:
        return f"-- ❌ Grafik Çizim Hatası: {str(e)}"

# --- 2. ANA ROTA (ENDPOINT) ---

@blueprint.route("/generate_sql", methods=["POST"])
@pga_login_required
def generate_sql():
    try:
        data = request.json
        ...
        conn = get_db_connection_ai(trans_id)
        if conn:
            # AI'nın hata yapmaması için gerçek tablo ve sütunları çekiyoruz
            sql_schema = """
                SELECT table_name, column_name 
                FROM information_schema.columns 
                WHERE table_schema='public' 
                AND table_name IN ('film', 'actor', 'language', 'category', 'customer', 'inventory', 'film_actor')
                ORDER BY table_name;
            """
            status, res = conn.execute_dict(sql_schema)
            if status and res and 'rows' in res:
                # Şemayı AI'ya çok daha okunaklı (Tablo: sütun, sütun) formatında sunalım
                schema_dict = {}
                for row in res['rows']:
                    t = row['table_name']
                    c = row['column_name']
                ...
                schema_info = "VERİTABANI ŞEMASI:\n"
                for table, columns in schema_dict.items():
                    schema_info += f"- {table} tablosu sütunları: {', '.join(columns)}\n"

        # GRAFİK MODU
        if "ciz" in prompt.lower():
            sql_prompt = f"{schema_info}\nİstek: {prompt}\nSadece SQL kodu döndür."
            sql_to_run = ask_ai_service("SQL Uzmanı", sql_prompt, provider, api_key)
            sql_to_run = extract_pure_sql(sql_to_run)
            
            df = get_data_as_df_ai(trans_id, sql_to_run)
            if df is not None and not df.empty:
                return jsonify({'success': 1, 'data': generate_chart_logic(df, prompt, provider, api_key)})
            else:
                return jsonify({'success': 0, 'errormsg': "Sorgu veri döndürmedi veya hatalı."})
        
        # NORMAL SQL MODU
        res = ask_ai_service(f"{schema_info}\nPostgreSQL Uzmanı", prompt, provider, api_key)
        if not res:
            return jsonify({'success': 0, 'errormsg': 'AI yanıt vermedi.'})

        return jsonify({'success': 1, 'data': extract_pure_sql(res)})

    except Exception as e:
        return jsonify({'success': 0, 'errormsg': str(e)})