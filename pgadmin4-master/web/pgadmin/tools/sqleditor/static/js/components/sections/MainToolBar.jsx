...
import AutoAwesomeIcon from '@mui/icons-material/AutoAwesome'; /* YENİ: AI İKONU */
...
import axios from 'axios'; /* YENİ: İstek atmak için */
...
/* --- YENİ: AI KONFİGÜRASYONU --- */
/* Buraya kendi API Key'ini yazabilirsin, ama Python tarafında hallediyorsan boş bırakabilirsin */
const GEMINI_API_KEY_JS = "AIzaSy..."; 
...
export function MainToolBar({containerRef, onFilterClick, onManageMacros, onAddToMacros}) {
  ...
  /* --- YENİ: AI FONKSİYONU --- */
  /* --- YENİ GELİŞMİŞ AI PANELİ (DOM MODAL) --- */
  const generateSQL = useCallback(() => {
    ...
    `;
    const boxStyle = `
        background: #2b2b2b; color: #e0e0e0; padding: 25px;
        border-radius: 12px; width: 550px;
        box-shadow: 0 10px 30px rgba(0,0,0,0.5); border: 1px solid #444;
        display: flex; flex-direction: column; gap: 15px;
    `;
    const inputStyle = `
        width: 100%; padding: 12px;
        background: #1e1e1e; border: 1px solid #3e3e3e; color: #fff;
        border-radius: 6px; font-size: 14px; outline: none;
        transition: border 0.3s;
    `;
    const labelStyle = `font-size: 12px; color: #aaa; font-weight: bold; text-transform: uppercase; margin-bottom: 5px; display: block;`;
    const btnStyle = `padding: 10px 20px; border: none; border-radius: 6px; cursor: pointer; font-weight: 600; font-size: 14px; transition: 0.2s;`;
    ...
    title.innerText = "Yapay Zeka Asistanı 🧠";
    ...
    selectLabel.innerText = "Mod Seçimi:";
    selectLabel.style.cssText = labelStyle;
    selectContainer.appendChild(selectLabel);

    let select = document.createElement('select');
    select.style.cssText = inputStyle;
    select.innerHTML = `
        <option value="generate">✨ Kod Üret (Generate)</option>
        <option value="fix">🔧 Hatayı Düzelt (Fix)</option>
        <option value="optimize">🚀 Hızlandır (Optimize)</option>
        <option value="explain">🧐 Kodu Yorumla (Explain)</option>
    `;
    selectContainer.appendChild(select);
    box.appendChild(selectContainer);

    // Prompt Kutusu
    let textContainer = document.createElement('div');
    let textLabel = document.createElement('label');
    textLabel.innerText = "İsteğiniz veya SQL Kodunuz:";
    ...
    textarea.placeholder = "Örn: 'Son eklenen 5 kullanıcıyı getir' VEYA Hatalı kodu yapıştır...";
    ...
    cancelBtn.style.cssText = btnStyle + "background: #444; color: #fff;";
    cancelBtn.onmouseover = () => cancelBtn.style.background = "#555";
    cancelBtn.onmouseout = () => cancelBtn.style.background = "#444";
    cancelBtn.onclick = () => document.body.removeChild(modal);
    btnGroup.appendChild(cancelBtn);

    // Editöre Aktar Butonu (Başlangıçta gizli)
    let applyBtn = document.createElement('button');
    applyBtn.innerText = "Editöre Aktar 📝";
    applyBtn.style.cssText = btnStyle + "background: #27ae60; color: #fff; display: none;";
    applyBtn.onclick = () => {
        // SQL'i Editöre yaz
        eventBus.fireEvent(QUERY_TOOL_EVENTS.EDITOR_SET_SQL, textarea.value);
    ...
    sendBtn.innerText = "Sihri Başlat ⚡";
    sendBtn.style.cssText = btnStyle + "background: #8e44ad; color: #fff;";
    sendBtn.onmouseover = () => sendBtn.style.background = "#9b59b6";
    sendBtn.onmouseout = () => sendBtn.style.background = "#8e44ad";
    ...
    sendBtn.onclick = async () => {
        ...
        if (!userPrompt) {
            ...
            setTimeout(() => textarea.style.border = "1px solid #3e3e3e", 2000);
            return;
        }
        ...
        try {
            let url = url_for('sqleditor.generate_sql');
            
            const postData = {
                prompt: userPrompt,
                action: select.value, // Frontend'den Backend'e modu gönderiyoruz
                trans_id: queryToolCtx.params.trans_id,
                api_key: GEMINI_API_KEY_JS,
                provider: 'ollama'
            };
            ...
            if (response.data.success === 1) {
                // Sonucu textarea'ya geri yaz
                textarea.value = response.data.data;
                
                // Başarılı olunca butonları güncelle
                sendBtn.style.display = "none"; // Gönderi gizle
                applyBtn.style.display = "block"; // Aktarı göster
                cancelBtn.innerText = "Kapat";
                
                // Başlığı güncelle
                title.innerText = "✅ İşlem Tamam!";
                title.style.color = "#27ae60";

            } else {
                alert("AI Hata: " + response.data.errormsg);
                sendBtn.innerText = "Tekrar Dene";
                sendBtn.disabled = false;
            }

        } catch (error) {
            console.error("AI Error", error);
            alert("Bağlantı Hatası: " + (error.response?.data?.errormsg || error.message));
            sendBtn.innerText = "Hata Oluştu";
            sendBtn.disabled = false;
        } finally {
    ...
    (<>
      <StyledBox>
        ...
        <PgButtonGroup size="small">
          ...
          {/* --- YENİ EKLENEN AI BUTONU (Play butonunun yanına) --- */}
          <PgIconButton 
            title="AI Generate SQL" 
            icon={<AutoAwesomeIcon style={{color: '#8e44ad'}} />} 
            onClick={generateSQL} 
